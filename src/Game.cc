...
static void updateTextureCubeRectBgra8(
    bgfx::TextureHandle _handle, uint8_t _side, uint16_t _x, uint16_t _y, uint16_t _width, uint16_t _height, uint8_t _r, uint8_t _g, uint8_t _b, uint8_t _a = 0xff)
{
    bgfx::TextureInfo ti;
    bgfx::calcTextureSize(ti, _width, _height, 1, false, false, 1, bgfx::TextureFormat::BGRA8);

    bgfx::Memory const* mem = bgfx::alloc(ti.storageSize);
...
bgfx::TextureHandle loadTextureWithUpdate(char const* _filePath, uint64_t _flags = BGFX_TEXTURE_NONE | BGFX_SAMPLER_NONE)
{
        ...
        if (NULL != imageContainer) {
            ...
            if (!imageContainer->m_cubeMap
                ...
                handle = bgfx::createTexture2D(
                    uint16_t(imageContainer->m_width), uint16_t(imageContainer->m_height), 1 < imageContainer->m_numMips, imageContainer->m_numLayers, bgfx::TextureFormat::Enum(imageContainer->m_format), _flags, NULL);

                bimg::ImageBlockInfo const& blockInfo = getBlockInfo(imageContainer->m_format);
                ...
                for (uint8_t lod = 0, num = imageContainer->m_numMips; lod < num; ++lod) {
                    ...
                    if (bimg::imageGetRawData(*imageContainer, 0, lod, imageContainer->m_data, imageContainer->m_size, mip)) {
                        uint8_t const* mipData = mip.m_data;
                        uint32_t mipDataSize = mip.m_size;

                        bgfx::updateTexture2D(
                            handle, 0, lod, 0, 0, uint16_t(width), uint16_t(height), bgfx::copy(mipData, mipDataSize));
                    }
...
void Game::init(int32_t _argc, char const* const* _argv, uint32_t _width, uint32_t _height)
{
    ...
    if (m_texture3DSupported) {
        bgfx::Memory const* mem8 = bgfx::alloc(32 * 32 * 32);
        bgfx::Memory const* mem16f = bgfx::alloc(32 * 32 * 32 * 2);
        bgfx::Memory const* mem32f = bgfx::alloc(32 * 32 * 32 * 4);
    ...
    m_textureCube[0] = bgfx::createTextureCube(
        kTextureSide, false, 1, bgfx::TextureFormat::BGRA8, BGFX_SAMPLER_MIN_POINT | BGFX_SAMPLER_MAG_POINT | BGFX_SAMPLER_MIP_POINT);
    ...
    if (m_computeSupported) {
        m_textureCube[2] = bgfx::createTextureCube(
            kTextureSide, false, 1, bgfx::TextureFormat::RGBA8, BGFX_TEXTURE_COMPUTE_WRITE);
    }
    ...
    m_texture2d = bgfx::createTexture2D(
        kTexture2dSize, kTexture2dSize, false, 1, bgfx::TextureFormat::BGRA8, BGFX_SAMPLER_MIN_POINT | BGFX_SAMPLER_MAG_POINT | BGFX_SAMPLER_MIP_POINT);
...
void Game::ImGuiDescription(float _x, float _y, float _z, float const* _worldToScreen, char const* _text)
{
...
bool Game::update()
{
    if (!entry::processEvents(m_width, m_height, m_debug, m_reset, &m_mouseState)) {
        ...
        ImGui::Begin(
            "Show descriptions", NULL, ImGuiWindowFlags_NoResize);
        ...
        if (now > m_updateTime) {
            ...
            if (m_cube.find(bw, bh, face)) {
                ...
                ++m_hit;
                Pack2D const& rect = face.m_rect;

                updateTextureCubeRectBgra8(m_textureCube[0], face.m_side, rect.m_x, rect.m_y, rect.m_width, rect.m_height, m_rr, m_gg, m_bb);
                if (m_blitSupported) {
                    bgfx::blit(
                        0, m_textureCube[1], 0, rect.m_x, rect.m_y, face.m_side, m_textureCube[0], 0, rect.m_x, rect.m_y, face.m_side, rect.m_width, rect.m_height);
                }
...

            {
                ...
                bgfx::Memory const* mem = bgfx::makeRef(dst, tw * th * 4);
        ...
        static char const* descTextureCube[BX_COUNTOF(m_textureCube)] = {
            "updateTextureCube",
            "blit",
            "compute",
            "frameBuffer",
        ...
        float const aspectRatio = float(m_height) / float(m_width);
        float const margin = 0.7f;
        float const sizeX = 0.5f * numColumns * 2.3f + margin;
        float const sizeY = sizeX * aspectRatio;
        ...
        float const xpos = -sizeX + margin + 1.0f;

        static char const* descTextures[] = {
            "create\nbc1",
            "create\nbc2",
            "create\nbc3",
            "create\netc1",
        ...
        static char const* descTextures3d[] = {
            "Tex3D R8",
            "Tex3D R16F",
            "Tex3D R32F",
        };
